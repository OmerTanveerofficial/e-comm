  ...
  return (
    <main className="container-main py-14">
      <div className="max-w-2xl mx-auto">
        ...
        <h1 className="font-serif font-bold text-3xl text-zinc-900 dark:text-zinc-100 mb-2">
          Track Your Order
        </h1>
        ...
        <form onSubmit={handleTrack} className="flex gap-3 mb-10">
          <input
            ...
            className="flex-1 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-amber-400 shadow-card"
          />
          <button
            type="submit"
            className="rounded-xl bg-zinc-900 dark:bg-zinc-100 px-6 py-3 text-sm font-semibold text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-200 transition-colors shadow-sm"
          >
            Track
          </button>
        </form>
        ...
        {tracked && (
          <div className="card p-6 sm:p-8">
              ...
              <div className="flex justify-between relative z-10">
                {steps.map((step, i) => (
                  <div key={i} className="flex flex-col items-center gap-2 flex-1">
                    ...
                    <p className={`text-[10px] text-center leading-tight font-medium ${step.done ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-600'}`}>
                      {step.label}
                    </p>
                  </div>
                ))}
            ...
            <p className="mt-8 text-xs text-zinc-400 dark:text-zinc-500 text-center">
              Last updated: Today at 9:41 AM · Questions? <a href="/contact" className="underline hover:text-amber-600">Contact us</a>
            </p>
          </div>
        )}
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
          {[
            { icon: '📦', title: 'Packed carefully', desc: 'Every order is packed with care' },
            { icon: '🚚', title: 'Fast delivery', desc: '2–5 business days standard' },
            { icon: '↩️', title: 'Easy returns', desc: '30-day no-hassle policy' },
          ].map(item => (
            <div key={item.title} className="rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
